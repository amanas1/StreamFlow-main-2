...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, language }) => {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            setStage('success');
            // Allow state to propagate naturally. 
            // If user is set, App.tsx will hide the modal.
            setTimeout(() => {
                // Optional: Check if we are still mounted/open
            }, 1000);
        } catch (err: any) {
                ...
                {stage === 'choice' && (
                    <div className="space-y-4">
                        <button 
                            ...
                            className="w-full py-4 bg-white text-slate-900 rounded-xl font-bold hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(255,255,255,0.2)]"
                        >
                        ...
                        <button 
                            ...
                            className="w-full py-4 bg-slate-800 text-white border border-white/10 rounded-xl font-bold hover:bg-slate-700 active:scale-95 transition-all text-sm"
                        >
